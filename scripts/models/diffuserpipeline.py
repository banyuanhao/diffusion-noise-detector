...
class StableDiffusionPipelineForNegativePrompts(
    ...
    def __call__(
        ...
        # byh
        negative_time: Optional[Union[List[int],int]] = None,
    ):
        r"""
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if do_classifier_free_guidance:
                    ...
                    elif type(negative_time) == list and i in negative_time:
                        noise_pred = noise_pred_negative + guidance_scale * (noise_pred_positive - noise_pred_negative)
                    elif type(negative_time) == int and i < negative_time:
                        noise_pred = noise_pred_negative + guidance_scale * (noise_pred_positive - noise_pred_negative)
                    else:
    ...
    def inject(
        ...
        cross_attention_kwargs: Optional[Dict[str, Any]] = None,
        # byh
        inject_step: List[int] = None,
        inject_guidance: float = None,
    ):
        r"""
        Function invoked when calling the pipeline for generation.
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if do_classifier_free_guidance:
                    noise_pred_negative, noise_pred_positive, noise_pred_uncond = noise_pred.chunk(3)
                    if i in inject_step:
                        noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_positive - noise_pred_uncond) - inject_guidance * (noise_pred_negative-noise_pred_uncond)
                    else:
                        noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_positive - noise_pred_uncond)
    ...
    def one_step_estimation(
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                for i, t in enumerate(timesteps):
                    ...
                    if estimate_time is not None and i == estimate_time:
                        latents = reverse_noise(self.scheduler, t, latents, noise_pred)
                        break
    ...
    def one_step_estimation_interwave(
            self,
            prompt: Union[str, List[str]] = None,
            height: Optional[int] = None,
            width: Optional[int] = None,
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                for i, t in enumerate(timesteps):
                    ...
                    noise_pred = self.unet(
                        latent_model_input,
                        t,
                        encoder_hidden_states = prompt_embeds if negative_time is None or i in negative_time else prompt_embeds_later,
                        cross_attention_kwargs=cross_attention_kwargs,
                    ).sample
...
class StableDiffusionPipeline(
    ...
    def encode_latents(self, image):
        image = torch.tensor(image).float().unsqueeze(0).permute(0, 3, 1, 2).to(self._execution_device)
        latents = self.vae.encode(image)
        print(dir(latents))
        print(latents.latent_dist)
    ...
    def __call__(
        ...
        if output_type == "latent":
            ...
            # 10. Convert to PIL
            image = self.numpy_to_pil(image)
        elif output_type == "latent and pil":
            # 8. Post-processing
            image = self.decode_latents(latents)
            # 9. Run safety checker
            image, has_nsfw_concept = self.run_safety_checker(image, device, prompt_embeds.dtype)
    ...
    def process_and_return_noises(
        ...
        pos_noises = []
        neg_noises = []      
        pos_noises.append(latents)
        neg_noises.append(latents)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if do_classifier_free_guidance:
                    noise_pred_uncond, noise_pred_text = noise_pred.chunk(2)
                    pos_noises.append(noise_pred_text)
                    neg_noises.append(noise_pred_uncond)
                    noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_text - noise_pred_uncond)
        ...
        return StableDiffusionPipelineOutput(images=image, nsfw_content_detected=has_nsfw_concept), pos_noises, neg_noises
