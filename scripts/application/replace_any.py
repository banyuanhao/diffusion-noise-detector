...
def generate_patch_sin(size):
    ...
    mat_x = torch.sin( 2 * torch.pi * x)
    mat_y = torch.sin( 2 * torch.pi * y)
    mat_z = torch.sin( 2 * torch.pi * z)
...
with torch.no_grad():
    ...
    x_t, y_t, width_t, height_t = bounding_box_latent_target
    latents_target[:, :, y_t:y_t+height_t, x_t:x_t+width_t] = generate_patch_sin((4,height_t, width_t))
