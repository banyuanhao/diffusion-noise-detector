...
for i in range(200):
    ...
    latents = torch.randn((1,4,height//8, width//8), generator=set_seed(seed), device='cuda', dtype=torch.float32)
    ...
    with torch.no_grad():
        ...
        out = pipe(prompt=prompt, latents = latents, height = height, width = width)
        ...
        values.append(iou)
    import json
    with open(f'/home/banyh2000/odfn/scripts/rebuttal/data/generalization/aspect_ratio_{ratio}_{mode}.json','w') as f:
        json.dump(values,f)
